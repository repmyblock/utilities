...
my $TableToQuery = "SELECT * FROM RepMyBlock.SystemUserTemporary WHERE SystemUser_ID is null AND SystemUserTemporary_createtime < DATE_SUB(NOW(), INTERVAL 96 HOUR)";
...
sub PrepareEmail {
  ...
  my $Frame = Frame->new();
	...
	${$_[1]}{'Subject'} = "Your RepMyBlock profile will be deleted tomorrow.";
...
<P>
	...
	You registered on the RepMyBlock website, but you did not complete the 
	registration after 96 hours; therefore, we will delete your profile tomorrow.
</P>

<P>
	If you are not interested in the RepMyBlock free services, you don't need to do anything. 
	We will be deleting your email address and your username in 24 hours. 
	<B>It's the last email you will receive from us.</B>
</P>

<P>
	Petitioning started on March 1<sup>st</SUP> and will go on until April 3<SUP>rd</SUP>. If you want to 
	help a candidate get on the ballot, all you need to do is log into RepMyBlock with your username 
	<I>"<B>$username</B>"</I> and the password 
...
<P>
	...
	password link</B>  to change your password. If you forgot your password, reply to this message, 
	and we'll delete your email so you can register again.
</P>
