...
void RawFilesInjest::NY_RawFilesInjest(void) { 
  FileName = "/home/usracct/VoterFiles/" + StateNameAbbrev + "/" + TableDate + "/AllNYSVoters_" + TableDate + ".txt";
}
...
void RawFilesInjest::NY_parseLineToVoterInfo(std::queue<std::string>& queue) {
  ...
  if (TableDateNumber < 20230213) {
    std::cout << HI_BLUE << "Entering NY for TableDateNumber " << NC << HI_YELLOW << TableDateNumber << NC << HI_BLUE << " < " << 20230213 << NC << std::endl;
    ...
    std::cout << HI_BLUE << "Entering NY for the else TableDateNumber " << NC << HI_YELLOW << TableDateNumber << NC << std::endl;
